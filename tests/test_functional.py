...
from mathtools.functional import argmax, argmin, identity, tuple_args
...
def test_argmax() -> None:
    ...
    assert argmax(tuple_args(f), f_args) == (3, 'this is a test', Foo(10))

def test_argmin() -> None:
    assert argmin(identity, ascii_lowercase) == 'a'
    assert argmin(lambda x: x * x, range(-5, 6)) == 0
    assert argmin(tuple_args(f), f_args) == (5, '', Foo(-1))
